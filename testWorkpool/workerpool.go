...
Loop:
	for {
		...
		case task, ok := <-p.taskQueue:
			...
			default:
				...
				if int(p.runWorks) < p.maxWorkers {
					fmt.Println("new worker")
					wg.Add(1)
					go worker(task, p.workerQueue, &wg)
					//atomic.StoreInt64(&p.runWorks, p.runWorks+1)
					p.runWorks++
				} else {
					// Enqueue task to be executed by next available worker.
					fmt.Println("add wait queue")
	...
	for p.runWorks > 0 {
		...
		fmt.Println("run works: ", p.runWorks)
	}

	fmt.Println("recycle wait....")
	wg.Wait()

	timeout.Stop()
}
func (p *WorkerPool) WorkSize() int {
	return int(p.runWorks)
}
...
func (p *WorkerPool) IsFullDone() bool {
	//fmt.Println("work size ", p.WorkSize())
	if !p.Stopped() && p.WorkSize() == 0 {
		return true
	}
	if p.Stopped() && p.WorkSize() == 0 {
